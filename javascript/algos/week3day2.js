...
const items = [
  ...
  function findObjects(criteria, collection) {
    return collection.filter(object => {
        // filter will iterate through the collection array
        let match = true;
        // will go through an array and checks if it's matched
        for (let key in criteria) {
          if (object[key] !== criteria[key]) {
            match = false;
            // if the value in object will not match criteria value it will break using break;
            break;
          }
        }
        return match;
