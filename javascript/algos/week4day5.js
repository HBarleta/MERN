...
function musicRuntime(busDuration, songDurations) {
    let temp = {};
    const songList = [...songDurations]
    songList.sort((a,b) => { return b-a});
    const duration = busDuration - 30
    songDurations.forEach((one, idx) => temp[one] = idx)
    
    for(let i = 0; i < songList.length; i++){
        const j = temp[duration - songList[i]]
        if(j){
            return [temp[songList[i]], j]
        }
    }
    return [-1, -1]
}
...
// 2 pointers that will check  all possible combinations that add up to 270
